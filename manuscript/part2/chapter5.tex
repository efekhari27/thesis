...
%Luc: la pr\'{e}sence de sigma^2 n'a pas de lien avec la stationarit\'{e}. 
%Joseph : Peut-être faudrait-il alors changer la lettre grecque utilis\'{e}e, car sigma est souvent associ\'{e} à un \'{e}cart-type, et donc sigma^2 à une variance, ce qui voudrait dire que K est un noyau de corr\'{e}lation.} 
...
The kernel used for KH is different and corresponds to the tensor product of one-dimensional Mat\'{e}rn kernels as in \eq{eq:Matern5/2}, so that the potentials $P_{\pi}(\cdot)$ are known explicitly (see Appendix~\ref{apx:B}); the correlations lengths are set to $\theta=0.2$ in test cases 1 and 3 ($d=2$) and to $\theta=0.7$ in test case 3 ($d=8$).
...
%\end{figure}
In the following, an existing Monte Carlo sample $\bZ_N$ of $N=10^3$ points in $\R^{53}$ is used, which corresponds to $53$ independent random input variables (see \citealp{ioobou10} for details). 
...
Each of the design methods considered for selecting a test set can have a downside. 
...
Finally, the efficient implementation of KH relies on analytical expressions for the potentials $P_{\pi}$, which are explicitly available for particular distributions. 
For uniform distributions on $[0, 1]$, a summary for various kernels is presented in \citet[Table 1]{pronzato_zhigljavsky_2020}. 
In the specific case of standard normal distributions with Matérn kernels (with $\eta=5/2$), the analytical expression of the potential is given in Appendix~\ref{apx:B}.  
More combinations of distributions and kernels are adressed in the thesis of \citet[p.66]{briol_phd_2019}. 
