...
\bea
\overline{\Delta}^2(\xi_n,\mu;\eta_m) &=& \E \left[ \left( \ISE_{\xi_n}(\eta_m) - \ISE_\mu(\eta_m) \right)^2 \right]\\
...
\bea
\overline{\Delta}^2(\xi_n,\mu;\eta_m)&=& \int_{\X^2} \E[\delta_m^2(\bx)\delta_m^2(\bx')]\,\dd(\xi_n-\mu)(\bx)\dd(\xi_n-\mu)(\bx') \,.
\eea
...
The main idea is to replace $\xi_n$, uniform on $\bX_n$, by a nonuniform measure $\zeta_n$ supported on $\bX_n$, $\zeta_n = \sum_{i=1}^n w_i \delta_{\bx^{(i)}}$ with weights $\wb_n = (w_1,\ldots,w_n)\TT$ chosen such that the estimation error $\overline{\Delta}^2(\xi_n,\mu;\eta_m)$, and thus $d_{\Kbarbar}^2(\zeta_n,\mu)$, is minimized. 
...
\begin{remark} 
The optimal estimator $\widehat Q_{n*}^2$ focused on weighting numerator of the coefficient of determination defined in \elias{ccc}. 
...
%The previous estimator applies optimal weights to a given test set with respect to a targeted measure $\mu$. 
...
The role of the reflected point $R(\bx)$ is to avoid selecting $\bx^{(i+1)}$ too close to the boundary of $\X$, which is a major problem with standard coffee-house, especially when $\X=[0,1]^d$ with $d$ large. 
...
The factor $\sqrt{2}d$ in \eq{eq:FSSF-fr} proposed in \citet{shang_apley_2020} sets a balance between distance to the design $\bX_i$ and distance to the boundary of $\X$. 
...
We shall use a different approach here, based on the greedy minimization of the MMD \eq{eq:energySP} for the candidate set $\iS=\bX_N$: at iteration $i$, the algorithm chooses
...
Greedy MMD minimization can be applied to other kernels than the distance kernel \eq{eq:kE}, see \cite{teymur_gorham_2021,pronzato_2021}. 
...
Figures~\ref{fig:uniform_validation_designs} and \ref{fig:normal_validation_designs} correspond to $\mu$ uniform on $[0,1]^2$ and $\mu$ the standard normal distribution $\SN(\0b,\Ib_2)$, with $\Ib_2$ the 2-dimensional identity matrix, respectively. 
...
This allows a good estimation of $Q_\mu^2$ (see \eq{eq:Q2th}) by a large Monte Carlo sample (with size $M=10^6$), which will serve as reference when assessing the performance of each of the other estimators. 
...
The kernel used for kernel herding is different and corresponds to the tensor product of one-dimensional Matérn kernels \eq{eq:Matern5/2}, so that the potentials $P_{\mu}(\cdot)$ are known explicitly (see Appendix~\ref{apx:B}); the correlations lengths are set to $\theta=0.2$ in test-cases 1 and 3 ($d=2$) and to $\theta=0.7$ in test-case 3 ($d=8$).
...
\end{figure}
We have access to an existing Monte Carlo sample $\bZ_N$ of $N=1\,000$ points in $\R^{53}$, that corresponds to $53$ independent random input configurations; see \cite{ioobou10} for details. 
...
The sum $\sum_{i=1}^n w_i^*$ of the optimal weights of support points and kernel herding \eq{Eq:weights} is shown in Figure~\ref{fig:catharec2_weights} (orange and blue curves, respectively). 
