...
Herin, the MMD is used to build a quadrature rule by sampling from a know measure. 
...
In BQ, the main object is the random variable $I_{\pi}(\xi_n)$. 
According to \cite{briol_oates_2019}, its distribution on $\R$ is the pushforward of $\xi_n$ through the integration operator $I_\pi(\cdot)$, sometimes called \emph{posterior distribution}: 
\begin{equation}
    I_{\pi}(\xi_n) = \int_{\iD_\bX} (\xi(\bx) | \by_n)  \dd \pi(\bx) = \int_{\iD_\bX} \xi_n(\bx)  \dd \pi(\bx) \,.
\label{eq:posterior}
\end{equation}
...
Therefore, the input measure $\pi$ is propagated through the conditioned Gaussian process to obtain the random variable $I_{\pi}(\xi_n)$, with distribution represented on the right plot (brown curve). 
...
Taking the random process $\xi_n$ as Gaussian conveniently implies that its posterior distribution $a_\pi(\xi_n)$ is also Gaussian. 
...
\begin{equation}
     \overline{y}_n^{\mathrm{BQ}} := \E\left[I_{\pi}(\xi_n) | \by_n \right] 
...
\begin{equation}
    \left(\sigma_n^{\mathrm{BQ}}\right)^2 := \var\left(I_{\pi}(\xi_n) \right) 
...
As in the one-dimensional example presented in \fig{fig:bayesian_quad}, the expected value of $I_{\pi}(\xi_n)$ expressed in \eq{eq:mean_post} is a direct estimator of the quantity of interest \eq{eq:quadrature_rule}. 
...
\begin{equation}
     \E\left[I_{\pi}(\xi_n)\right] = \beta_n + P_\pi(\bX_n) \bK_n^{-1} \left(\by_n - \beta_n \mathbf{1}_n \right).
...
The link between the posterior variance and the squared MMD has been first made by \cite{husar_duvenaud_2012} in their Proposition 1: the expected variance in the Bayesian quadrature $\var\left(I_{\pi}(\xi_n)\right)$ is the MMD between the target distribution $\pi$ and $\zeta_n = \sum_{i=1}^n \wBQ^{(i)} \delta(\bx^{(i)})$. 
The proof is reproduced below (as well as in Proposition 6.1 from \cite{motonobu_2018}): 
\begin{subequations}
\begin{align}
    \var\left(I_{\pi}(\xi_n)\right) &= \E\left[ \left(I_{\pi}(\xi_n) - I_{\zeta_n}(\xi_n)\right)^2\right]\\
    &= \E\left[ \left( \left\langle \xi_n, P_{\pi} \right\rangle_{\iH(k)} - \left\langle \xi_n, P_{\zeta_n}\right\rangle_{\iH(k)} \right)^2\right]\\
    &= \E\left[ \left\langle \xi_n, P_{\pi} - P_{\zeta_n} \right\rangle_{\iH(k)}^2\right]\\
    %&= \E\left[ \lVert \xi_n \lVert^2 + \lVert P_{\pi}(\bx) - P_{\zeta_n}(\bx) \lVert^2 - 2 \left\langle \xi_n, P_{\pi}(\bx) - P_{\zeta_n}(\bx) \right\rangle_{\iH(k)}\right]\\
...
Later, \cite{briol_2015} proved the weak convergence of $I_{\pi}(\xi_n)$ towards the target integral. 
