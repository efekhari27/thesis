...
Second, those that rely on the use of surrogate models (or metamodels, see \fig{fig:UQ}) to emulate the costly numerical model by a statistical model. 
...
During the overall process described in \fig{fig:UQ}, modelers and analysts often need to determine whether inputs are influential or not in order to prioritize their effort (in terms of experimental data collecting, simulation budget, or expert elicitation). 
...
All these questions are intimately related to the topic of sensitivity analysis \citep{saltelli_2008,daveiga_iooss_2021} and can be seen as an ``inverse analysis'' denoted by Step C' in \fig{fig:UQ}. 
...
he considered use case here consists of a DIEGO model of a Siemens SWT 2.3MW bottom-fixed turbine on a monopile foundation (see the datasheet in Table \ref{tab:owt_datasheet}), currently operating in Teesside, UK (see the wind farm layout and wind turbine diagram in \fig{fig:teesside_layout}). 
...
The matrix plot of the transformed data presented in \fig{fig:envi_pairplot} is (to the best of the authors' knowledge) a new kind of plot named \emph{copulogram}. 
...
\begin{figure}[!h]
    ...
    Scatter plots on the bottom triangle are set in the physical space. \elias{Legend is cropped}}
...
The  Sklar theorem \citep{joe_1997} states that the multivariate distribution of any random vector $\mathbf{X} \in \R^p, p \in \N^*$ can be broken down into two objects:
\begin{enumerate}
...
Provided a large enough learning set $\bX_n$ (over five years in the present case), the EBC combined with kernel density estimation for the marginals enable to fit well the environmental joint distribution related to the dataset in \fig{fig:envi_pairplot}. 
...
As defined by the DNV standards for OWT fatigue design \citep{dnv_fatigue_2016}, the quantity of interest in the present paper is the ``mean damage'' $d_{i, \theta}$ (also called ``expected damage''), computed at a node $i$, for an azimuth angle $\theta$:
\begin{equation}
...
When $P_{\zeta_n}(\bx)$ is close to $P_{\mu}(\bx)$, it can be interpreted as $\zeta_n$ being an adequate quantization or representation of $\mu$ (which leads to a good estimation of a quantity such as $I_{\mu}(g)$ from \eq{eq:quadrature_rule}). 
...
\begin{table*}[ht] \caption{Kernels considered in the following numerical experiments.}
    \begin{center}
    %\tabletext{
...
Let us recall the probabilistic integration problem $I_\mu(g) = \int_{\iD_\bX} g(\bx) \dd \mu(\bx)$ (stated in \eq{eq:quadrature_rule}). 
...
%To ease the reading, the results from the other kernels defined in Table \ref{tab:kernels} are moved to \elias{Appendix \ref{apx:ana_bench}}.
