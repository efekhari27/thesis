...
\begin{remark}
    ...
    \begin{subequations}
        \begin{align}
            ...
            \Leftrightarrow \quad c\left(F_{X_1}(x_1), F_{X_2}(x_2)\right) &= \frac{f_{X_2\, |\, X_1}(x_2\, |\, x_1)}{f_{X_2}(x_2)}.
        \end{align}  
    \end{subequations}
\end{remark}
\medskip
...
Considering a sample $\bX_n = \left\{\bx^{(1)}, \dots, \bx^{(n)}\right\} \sim \P_{\bX}$ with the respective ranks $\bR_n = \left\{\br^{(1)}, \dots, \br^{(n)}\right\}$, the associated empirical copula is defined as: 
\begin{equation}
...
\begin{equation}
    1 = \sum_{t=0}^{m} b_{m, t}(u), \quad \forall u\in\R, \quad \forall m \in \N.
\end{equation} 
...
In this section, a hybrid inference strategy is presented, coupling some well-known parametric models for the marginals (e.g., Weibull distribution for the wind speed), with a flexible dependence modeling by the EBC. 
...
To confirm this first impression, statistical Kolmogorov–Smirnov tests successfully validated the adequacy of the two parametric models. 
...
\begin{table}[ht]
    ...
    \begin{tabular}{lll}
        \hline
        {\it Name}              & {\it Notation}            & {\it Fitted model}                            \\
        \hline
        Wind speed              & $U$                       & Weibull ($\beta=11.4, \alpha=2.2, \gamma=0$)  \\
        Wind direction          & $\theta_{\mathrm{wind}}$  & KDE                                           \\ 
        Significant wave height & $H_s$                     & Inverse Normal ($\mu=2.3, \lambda=6.8$)       \\
        Wave period             & $T_p$                     & KDE                                           \\
...
However, despite its apparent flexibility, this tool should not be used carefully when inferring the distributions' tails. 
...
Then, the MMD was either used for testing the goodness-of-fit of the hybrid model based on the EBC, or as a measure of the perturbation occasioned by the wake on the wind conditions. 
