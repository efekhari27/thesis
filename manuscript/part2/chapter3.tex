...
In our case, this method not only gathers turbines with the same wind conditions, but it also determines a representative turbine for each cluster. 
...
Interestingly, the conclusion that emerged from comparing the wake-perturbed distributions to the ambient one (see \fig{fig:FIGWakeEffect}) are different from the ones obtained by comparing pairs of wake-pertubed distributions. 
...
In a context of fatigue assessment at the farm scale, the computed clusters may reduce the computational effort by assessing one turbine per cluster.    
...
First, to infer a joint environmental distribution, a semiparamertic approach was applied to a large dataset from a site in South Brittany, France. 
This semiparamertic approach combines parametric models of the marginals with nonparametric models of the copula such as the highly flexible empirical Bernstein copula.  
...
Either for testing the goodness-of-fit of the semiparamertic model fitted using the EBC, or as a measure of the perturbation occasioned by the wake on the wind conditions. 
