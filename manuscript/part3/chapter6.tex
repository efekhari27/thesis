...
The assessment of a system's reliability, such as offshore wind turbines, often involves the estimation of rare event probabilities. 
The system's performance is typically modeled by a deterministic scalar function, denoted by $g: \iD_\bx \subseteq \R^d \rightarrow \R$, referred to as the \textit{limit-state function}. 
...
Considering a probabilistic framework, inputs' uncertainties are modeled by a continuous random vector $\bX \in \iD_\bx$, with a joint probability density function (PDF) denoted by $f_\bX$. 
...
A common risk measure in reliability analysis is the \textit{failure probability}, denoted by $\pf$, representing the probability of the system exceeding the threshold $\yth$:
\begin{equation}
    \pf := \P(g(\bX) \leq \yth)
...
Moreover, their validation as well as their behavior with respect to large input dimension case make also their use quite complex (see, e.g.,  \citep{marrel_iscream_2022}).
...
To overcome this limit, advanced simulation techniques have been developed: among others, one can mention several ``variance reduction methods'' such as the non-adaptive and adaptive versions of the \emph{Importance Sampling} \citep{RubinsteinKroese1981} 
(either parametric, using the Cross-Entropy method \citet{NKurtz_JSong_StrucSaf_2013}, or nonparametric \citet{Morio_RESS_2011}) and splitting techniques \citep{cerou2012sequential} such as the \emph{Subset Simulation} (SS) \citet{AuBeck2001}. 
...
In adaptive importance sampling\elias{, summarize the limits of AIS-CE and NAIS.}
...
%Among others, one can refer to the ``adaptive importance sampling by cross-entropy'' \citep{NKurtz_JSong_StrucSaf_2013}, or the ``nonparametric adaptive importance sampling'' \citep{Morio_EJOP_2010}. 
%Alternatively, ``subset sampling'' (SS) \citep{AuBeck2001} (also referred to as ``multilevel splitting'' \citep{glasserman1999multilevel}, or ``sequential Monte Carlo'' \citep{cerou2012sequential}) is a different type of variance reduction method. 
...
Further discussions and literature review on this topic are presented in Setion~\ref{sec:rare_event} of this manuscript. 


\elias{Compare with NAIS, not SS.}
\elias{Say that this work improves and extends the ICASP paper}
...
In this chapter, Section 2 will recall the methodology of subset sampling and probabilistic modeling. 
...
%============================================================%
\begin{enumerate}
    \item The algorithm / estimator + variance / illustrations 2D
    \item Optimal tuning for empirical Bernstein copula inference 
    \item Code available on GitHub
\end{enumerate}
...
\State Compute IS weights $\bw_{[k+1], N} = \{w_{[k+1]}^{(j)}\}_{j=1}^N = \left\{\frac{f_\bX(\bx_{[k+1]}^{(j)})}{\widehat{f}_{[k+1]}(\bx_{[k+1]}^{(j)})}\right\}_{j=1}^N$
...
\State Estimate $\widehat{\pf} = \frac{1}{N\, k_\#} \sum_{k=1}^{k_\#} \sum_{j=1}^N \1_{\{g(\bx_{[k]}^{(j)}) \geq \yth  \}}(\bx_{[k]}^{(j)}) \, w_{[k]}^{(j)}$
\State Estimate $\var(\widehat{\pf}) = \frac{1}{N-1} \left[\frac1N \sum_{j=1}^N \1_{\{g(\bx_{[k]}^{(j)}) \geq \yth  \}}(\bx_{[k]}^{(j)}) \, w_{[k]}^{(j)} - (\widehat{\pf})^2\right]$
\State \Comment{Outputs:}\hfill~
