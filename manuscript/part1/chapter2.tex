...
For example, computational fluid dynamics (\abv{cfd}) models give a detailed description of the wake (including near the turbine) but require high computational efforts. 
...
\begin{itemize}
    ...
    \item \textbf{\abv{diego}}: a multiphysics wind turbine model in the time domain (see Section~\ref{sec:owt_modeling}); 
...
\end{figure}
\textit{Blade element momentum theory} (\abv{bemt}) combines the results from blade element theory in \eq{eq:blade_element} with the results from momentum theory in \eq{eq:momentum_theory} to obtain the induction factors $a$ and $a'$. 
...
Then, the \textit{stress-number of cycles curve} of a specific material (\abv{sn} curve) gives the number of cycles before failure at a given constant stress amplitude. 
...
Following the terminology in civil engineering, the structure is designed for ``fatigue limit states'' (\abv{fls}) and ``ultimate limit states'' (\abv{uls}) with respect to environmental conditions. 
...
In accordance, the costs of handling operations and maintenance represent an important share of the \textit{levelized cost of energy} (\abv{lcoe}) \citep{shields_2021_owt_lcoe}.  
...
\begin{table}[h!]
    ...
    \begin{tabular}{llll}
        ...
        S-N curve coeff. & $\varepsilon$ & Log-normal($\mu=1, \sigma=0.2$) & See \cite{guede_2007}\\
