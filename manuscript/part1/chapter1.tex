...
\begin{theorem}[Sklar's theorem]
    ...
    The reader might refer to \elias{cite proof} for further mathematical proof. 
    \label{thm:sklar}
\end{theorem}
...
\elias{Add reviews from the appendix A in Bertrand's book}
...
\begin{itemize}
    ...
    \item \elias{Add the summary table with the properties of each method?}
\end{itemize}
...
one can apply the change of variable $\bx = T(\bu)$ to express the reliability problem from \elias{add pointer} in the standard space: 
\begin{equation}
    \pf = \P(\cg(\bU) \leq 0) 
...
\elias{Discussion on common advantages and drawback of the IS methods.}
...
\elias{Mention the Appendix \ref{apx:C} presenting the algorithms of SS}
...
\elias{Nonlinearities of the LSF are the actual challenge in reliability analysis}
...
The aim of sensitivity analysis (SA) is to determine the impact of a single random input (or a group of random inputs) on a random output. 
As described earlier, this step is qualified as an inversion problem in the general UQ framework \elias{add pointer}, in opposition with the forward uncertainty propagation step. 
In fact, the analyst studies the impact of inputs on a QoI of an output. 
Depending on the type of QoI, the SA is quialified as ``global'' or ``local''. 
Global sensitivity analysis (GSA), typically studies the general variability (e.g., the variance) of the output, while local SA can focus on the impact of inputs on a local statistic (e.g., quantile, failure probability). 
Two types of GSA methods exist in the literature: qualitative and quantitative approaches.  
Each answer two different questions in UQ: 
\begin{itemize}
    \item \textit{screening}: determine the non-influential variables in a UQ study (qualitative);
    \item \textit{importance measure}: assess the contribution of input(s) in the global variability of the output (quantitative).
\end{itemize}
\elias{Disclaimer: SA on dependent inputs are an open problem and the inputs will be considered as independent in the following.}
...
The Morris method \elias{add ref Morris 91} is screening method historically commonly used in engineering applications. 
It starts by mapping the input domain $\iD_{\bX}$ to a unit hypercube $[0, 1]^d$, which is discretized as a regular grid with step $\Delta \in \R$.
...
Each OAT designs start at a random node $\bx^{(t)} = (x^{(t)}_1, \dots, x^{(t)}_i,\dots, x^{(t)}_d)$ of the grid and move only in one direction by an increment equal to the elementary step such that: $\bx^{(t)} + \Delta_i = (x^{(t)}_1, \dots, x^{(t)}_i + \Delta ,\dots, x^{(t)}_d)$. 
The elementary effect of an OAT design $t$ in the direction of the variable $i$ is expressed as a finite difference: 
\begin{equation}
    \mathrm{EE}_i^{(t)} = \frac{g(\bx^{(t)}) - g(\bx^{(t)} + \Delta_i)}{\Delta}.
    \label{eq:eeffect}
\end{equation}

The Morris method generates $T\in\N$ OAT designs and computes theirs respective elementary effects in each direction $i$. 
To assess the global sensitivity of the function, the mean $\bar{\mathrm{EE}}_i$ and variance $\what{\var}(\mathrm{EE}_i)$ of the elementary effects are computed: 
\begin{equation}
    \bar{\mathrm{EE}}_i = \frac1n \sum_{t=1}^{T} |\mathrm{EE}_i^{(t)}|, \qquad 
    \what{\var}(\mathrm{EE}_i) = \frac{1}{n-1} \sum_{t=1}^{T} \left(\mathrm{EE}_i^{(t)} - \bar{\mathrm{EE}}_i\right)^2.
\end{equation}  


It divides the variables into three categories, regardless of any regularity hypothesis on the function: 
...
\elias{Impossible to distinguish interactions and nonlinear effects}
...
\elias{Reduced cost for screening. Bounding properties compared to a quantitative approach.}
\elias{Different rankings between DGSM and Sobol' obtained for nonlinear functions.}

\begin{equation}
    v_i = \int_{\iD_{\bX}} \left(\frac{\partial g(\bx)}{\partial x_i}\right)^2 f_{\bX}(\bx) = \E\left[\left(\frac{\partial g(\bX)}{\partial X_i}\right)^2\right].
\end{equation}
...
The following presentation is inspired by the paragraph 4.2.3.1 from \elias{Chabridon these}.  

Considering a squared-integrable function $g(\cdot): [0, 1]^d \rightarrow \R$\elias{add footnote for any input domain see Chastaing these} and the independent random vector $\bX$\elias{Add footnote for dependent variables see Chastaing}. 
The output random variable $Y = g(\bX)$ can be decomposed, according to \elias{Hoffeding 1948}, as: 
\begin{equation}
    Y = g(\bX) = g_0 + \sum_{i=1}^{d} g_i(X_i) + \sum_{i<j}^{d} g_{ij}(X_i, X_j) + \dots + g_{1 \dots d}(\bX),
    \label{eq:hoeffding}
\end{equation}
with the previous terms defined according this recurrence:  
\begin{align}
    g_0 &= \E[g(\bX)]\\
    g_i(X_i) &= \E[g(\bX) | X_i] - g_0\\
    g_{ij}(X_i, X_j) &= \E[g(\bX) | X_i, X_j] - g_i(X_i) - g_j(X_j) - g_0\\
    \dots
\end{align}
\elias{Sobol 93} proved that this decomposition is unique by exploiting the orthogonality of the terms of the decomposition. 
...
\begin{equation}
    \var(Y) = \sum_{i=1}^{d} V_i(Y)  + \sum_{i<j}^{d} V_{ij}(Y) + \dots + V_{1 \dots d}(Y), 
    \label{eq:fanova}
\end{equation}
with the previous terms defined in a recurrent way as $V_i(Y) = \var\left(\E[Y|X_i]\right), V_{ij}(Y) = \var\left(\E[Y|X_i, X_j]\right) - V_i(Y) - V_j(Y)$, and so on for higher order interaction terms. 
The Sobol' indices of different order are defined as normalized shares of variance. 
The first-order Sobol' index $S_i$ quantifies the share of variance of the output only explained by the marginal $X_i$ (also called main effect). 
Second order $S_{ij}$ (or higher order) Sobol' indices quantify the effect of the interactions between a group of marginals. 
\begin{align}
    S_i &= \frac{V_i(Y)}{\var(Y)} = \frac{\var\left(\E[Y|X_i]\right)}{\var(Y)}\\
    S_{ij} &= \frac{V_{ij}(Y)}{\var(Y)} = \frac{\var\left(\E[Y|X_i, X_j]\right) - V_i(Y) - V_j(Y)}{\var(Y)}\\
    \dots
...
\begin{equation}
    \sum_{i=1}^{d} S_i + \sum_{i<j}^{d} S_{ij} + \dots + S_{1 \dots d} = 1.
\end{equation}

Assessing Sobol' indices for every order becomes complex in medium to high dimension. 
The total Sobol' index $S_i^T$, see \elias{Saltelli book 2008}, quantifies the share of output variance which is explained by all the interactions of the marginal $X_i$:
\begin{equation}
    S_i^T = 1 - \frac{\var\left(\E[Y|\bX_{-i}]\right)}{\var(Y)} = \frac{\E\left[\var(Y|\bX_{-i})\right]}{\var(Y)},
    \label{eq:sobol_total}
\end{equation}
where $\bX_{-i}$ represents all the marginals from $\bX$ but $X_i$.

\elias{Note that $\forall i, S_i \leq 1$ and $S_i \leq S_{T_i}$. 
The total indexes are only equal to the first indexes when the function in purely additive.} 

\elias{The pick-freeze scheme is the historical estimation technique, however, it requires many calls to the function.}

\elias{The use of to polynomial chaos expansion to surrogate a small dataset provides a direct expression of the Sobol' indices, Add Marrel for GP.}

\elias{Advantages of Sobol': good quantification of the global sensitivity of the inputs}

\elias{Warning: the variance is not always a relevant description of the output distribution (e.g. multimodal distributions).}

%------------------------------------------------------------%
\subsubsection{Shapley effects}
%------------------------------------------------------------%
Shapley effects are an adaptation of Shapley values of cooperative games (\elias{Shapley 1953}) to GSA by \elias{Owen 2014}. 
Unlike Sobol' indices, this measure of importance does not rely on a decomposition of the output's variance. 
The generic Shapley effect of the variable $X_i$ is given considering the indices $-\{i\}= \{1, \dots, d\} \backslash \{i\}$:
\begin{equation}
    Sh_i = \sum_{v \subseteq -\{i\}} \frac{(d - |v| -1)! |v|!}{d!} \left(c(v \cup \{i\} - c(v))\right),
\end{equation}
with the cardinality of $v$ denoted $|v|$, and a cost function $c(\cdot)$. 
Conceptually, this expression compares a performance defined by a cost function with or without the variable $X_i$, and averages it over all the possible combinations of inputs.
Various contributions connected this measure of importance to the Sobol' indices by defining a cost function based on the output variance. 
In the case of independent inputs, the Shapley effects are bounded by the Sobol' indices $\forall i \in \{1, \dots, d\}$: 
\begin{equation}
    S_i \leq Sh_i \leq S_i^T.
\end{equation} 

\elias{Add analysis in the dependent case from Marouane's paper.}


%============================================================%
\subsection{Moment-independent importance measures}
%============================================================%
Beyond variance-based GSA, many divergences, discrepancies have been used to evaluate the dependence between the input and the output distributions. 
Considering the probability measures $\P_{X_i}$ and $\P_{Y}$, associated with the random variables $X_i$ and $Y$, two formulations exist:
\begin{itemize}
    \item \textit{dissimilarity measures}: comparing $\P_Y$ with $\P_{Y |X_i}$;
    \item \textit{dependence measures}: comparing $\P_{(X_i, Y)}$ with $\P_{X_i} \otimes \P_Y$.
\end{itemize}
These two tools are flexible and allow to plug any divergence, discrepancy between probability distributions. 
Dissimilarity measure based on f-divergence were studied in \elias{Da veiga 2015, Rahman 2016}, which generalized the dissimilarity measure based on the total-variation distance introduced by \elias{Borgonovo 2007}.

The following section focuses on dependence measures, which are generically defined as:
\begin{equation}
    S_i^{DM} = \Delta(\P_{(X_i, Y)}, \P_{X_i} \otimes \P_Y),
\end{equation}
where $Delta(\cdot)$ is a distance between two probability distributions. 

In particular, the \textit{Hilbert-Schmidt Independence Criterion} (HSIC) was initially introduced by \elias{Gretton 2006} for dependence testing, and adapted as a dependence measure in GSA by \elias{Da Veiga 2015}. 
This method relies on a distance between the embeddings of two distributions in a Hilbert space named \textit{maximum mean discrepancy} (MMD).
...
Considering the pair of random variables $(X_i, Y)$, with probability distributions $\P_{X_i}$ and $\P_Y$ let us define the RKHS $\iH$ 
induced by the product kernel $k((x_i, y), (x_i', y')) = k_{X_i}(x_i, x_i') k_{Y}(y_i, y_i')$. 
The HSIC between the variable $X_i$ and $Y$ is defined as the following dependence measure:
\begin{equation}
    \mathrm{HSIC}(X_i, Y) = \MMD(\P_{(X_i, Y)}, \P_{X_i} \otimes \P_{Y})^2
\end{equation}
...
Gaussian processes (GP) \citep{rasmussen_2006}, low-rank tensor approximations \elias{add ref}, and artificial neural network (ANN) \citep{tibshirani_2009}.
...
\elias{Comment the following figure}
...
\begin{remark}
    ...
    However, active learning is also used to handle big data frameworks in the machine learning community \elias{add ref}. 
...
The use of Gaussian processes as probabilistic surrogates for optimization was popularized by the \textit{efficient global optimization} (EGO) algorithm \elias{Jones (1998)}. 
...
Numerous reviews of this field were proposed by \elias{Shahriari 2016, Gramacy book 2020 Chapter 7} and numerical benchmarks presented in \elias{Leriche Picheny 2021}.
...
A first surrogate $\xi_n(\bx) \sim \iN(\eta_n(\bx), s_n^2(\bx))$ is fitted using \eq{eq:kriging}. 
...
Bayesian optimization is an active research field, with different open problems such as constrained Bayesian optimization \elias{S.Petit}, or Bayesian optimization on stochastic functions \elias{Gramacy, M.Binois?, Spagnol?}

%\citet{fau_2021} reviews adaptive sampling methods for Kriging. \elias{Cite Petit, Spagnol?}
\begin{figure}[ht]
    \centering
...
Rare event estimation often requires large amounts of calls to the limit-state function (becoming intractable for costly numerical model). 
...
Note that active learning procedure using SVM classifiers were adapted to this specific goal \elias{cite Bourinet}. 

The following paragraph introduces the most popular kriging-based learning criterion: the deviation number $U$ \elias{Echard 2012}. 
\elias{Morio et Balesdant 2015} introduce further active learning techniques dedicated to rare event estimation. 
\elias{Mustapha 2021 and Teixeira 2020} review this topic and present wide numerical benchmarks.

Considering an initial learning set $\left\{\bX_n, \by_n\right\}$ built on a space-filling input design $\bX_n$ to explore the domain. 
A first Gaussian process $\xi_n(\bx) \sim \iN(\eta_n(\bx), s_n^2(\bx))$ is fitted using \eq{eq:kriging}. 
...
\elias{Note that the calibration error is often larger than the surrogate model error.}
