...
A generalized discrepancy between distributions called \textit{maximum mean discrepancy} is introduced in the Appendix \ref{apx:D} and used for different purposes in the \elias{Part 2} of this manuscript.
...
Identically to the Monte Carlo variance in \eq{eq:MC_variance}, LHS variance can be expressed as:
\begin{equation}
...
Because of its stratified structure, LHS can generate poor designs from a space-filling point of view (see e.g., \fig{fig:poor_LHS}). 
...
This question will be further discussed using a more general framework in the \elias{Chapter 4}.
...
Considering the problem of exceeding a safety threshold in \eq{eq:failure_proba}, the system's performance is commonly defined as the difference between the model's output and a safety threshold $\yth\in\R$. 
...
The example illustrated in the \fig{fig:multi_FORM} has shown that the method might miss some important areas of the failure domain, leading to poor estimations. 
...
An optimal instrumental distribution $h_{\bX}^*$ theoretically gives the smallest variance by setting it equal to zero in \eq{eq:IS_variance}: 
\begin{equation}
...
This strategy is simple to implement, but it inherits the main drawbacks of FORM, such as the problems with multiple failure areas (see the example illustrated in \fig{fig:multi_FORM}).
...
%------------------------------------------------------------%
\subsubsection{\elias{Adaptive importance sampling}}
%------------------------------------------------------------%
...
\elias{Nonparametric adaptive importance sampling}
...
Bayesian optimization is an active research field, with different open problems such as constrained Bayesian optimization \citep{petit_2022}, or Bayesian optimization on stochastic functions \elias{cite Gramacy, M.Binois?}.
