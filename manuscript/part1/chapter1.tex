...
Finally, no-code users can benefit from \texttt{OpenTURNS}'s free-download Graphical User Interface software, named \texttt{Persalys}\footnote{\texttt{Persalys}, a free-download graphical user interface available at \url{https://www.persalys.fr/obtenir.php}}. 
...
For example, imprecise probability theory allows more general modeling of the uncertainties. \elias{add ref}
...
\begin{theorem}[Sklar's theorem]
    ...
    The reader might refer to \cite{joe_2014} for further mathematical proof. 
    \label{thm:sklar}
\end{theorem}
...
\fig{fig:univariate_quads} illustrates the nested properties of Fejér-Clenshaw-Curtis quadrature by representing the nodes of quadratures rules with increasing size.  
...
Using the Smolyak reccurent formula (se e.g., \citet{sullivan_2015}), two univariate quadratures can be combined as illustrated on the right of \fig{fig:bivariate_quads}. 
...
A generalized discrepancy between distributions called \textit{maximum mean discrepancy} is introduced in the Appendix \ref{apx:D} and used for efficient sampling in the \elias{Part 2} of this manuscript.
...
This question will be further discussed in the \elias{Chapter 4}, using a more general framework.
...
\fig{fig:IS_reliability} illustrates on the same two-dimensional case, the estimation by Monte Carlo and importance sampling centered on the design point. 
...
\end{equation}
By injecting the expression in \eq{eq:KL}, the optimization problem simply becomes function of an expected value over the initial density $f_\bX$:
\begin{equation}
...
As for the CE-IS methods, the risk is that barely any points sampled from the instrumental density $h_0$ hit the failure domain, leading to poor estimates. 
...
Rare event estimation is a generic problem that requires tailored solutions. 
Depending on the properties of the problem tackled, some methods might outperform others. 
Many more methods are worth mentioning in the field of reliability analysis, such as the \textit{directional sampling} \citet{bjerager_1988_directional_sampling}, or \textit{line sampling} \citep{koutsourelakis_2004_line_sampling}, \textit{moving particles} \citep{walter_2015_moving_particles}. 
\citet{MorioBalesdent2015} compares the advantages and drawbacks of the most commonly used one, with a systematic algorithmic description and numerical benchmark. 

Overall, the main properties increasing the complexity of reliability problems are related to:
\begin{itemize}
    \item the strong nonlinearity of the LSF;
    \item the rareness of the failure event;
    \item the computational cost of the evaluation of the numerical model.
\end{itemize} 
...
Considering all these elements the analyst may set up a sampling strategy, possibly coupled with the use of a surrogate model (see Section \ref{sec:surrogate}). 
...
The ``black-box reliability challenge'', was organized as a hackathon in 2019 by the Dutch organization for applied scientific research (TNO) \citep{rozsas_2019_bbrc}. 
This platform proposed a large catalog of reliability problems and their respective solutions. 
Most of them were encapsulated as a Python package called \texttt{otbenchmark} \citep{fekhari_baudin_2021}, based on core \ot objects.  
...
As described earlier, this step is qualified as an inverse analysis in the general UQ framework (illustrated in \fig{fig:UQ_methodo}), in opposition with the forward uncertainty propagation step. 
...
The reader may refer to \elias{add feature selection review} for further insights on this topic.
...
Estimating Sobol' indices can be achieved in various ways. 
...
\elias{Add analysis about the FANOVA-like decomposition.}

\elias{The simplicity of the estimation.}

\elias{Links with Sobol indices for a simple kernel.}
...
Multiple techniques resolve this issue by applying compression schemes on this matrix, e.g., based on sparse approximations (e.g., Hierarchical Matrices \citet{geoga_2020_hmat_gp}). 
...
\begin{otexample}[Ordinary kriging]
    ...
    Figures illustrating the present section may be reproduced, using the \ot scripts available on GitHub\footnotemark[10]\textsuperscript{,}\footnotemark[11].  
\end{otexample}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnotetext[10]{\url{https://github.com/efekhari27/thesis/blob/main/numerical_experiments/chapter1/surrogates.ipynb}}

\footnotetext[11]{\url{https://github.com/efekhari27/thesis/blob/main/numerical_experiments/chapter1/active_learning.ipynb}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
