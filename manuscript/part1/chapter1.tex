...
Among the methods presented so far, classical deterministic quadratures are subject to the curse of dimension while Monte Carlo methods deliver contrasted performances. 
...
In which the \textit{$L_p$ star discrepancy} of a design $\bX_n$ is defined as the $L_p$-norm of the difference between the empirical CDF of the design $\what{F}_{\bX_n}$ and the CDF of the uniform distribution $F_{\textbf{U}}$:  
\begin{equation}
...
Similarly to the Monte Carlo variance in \eq{eq:MC_variance}, LHS variance can be expressed as:
\begin{equation}
...
Once the first MPFP $P^{*^{(1)}}$ is found, the LSS is modified by removing a nudge to find to following MPFP $P^{*^{(2)}}$, positioned at a similar distance but in a different direction. 
...
Provided that the failure probability is bounded, this estimator converges toward it almost surely according to the LLN. 
...
However, many authors tried to adapt Sobol's indices to this context. 
...
Let us first define a prior structure $G$ on the function approximated $g(\cdot)$, taken as a Gaussian process with a mean function $m(\cdot)$ and covariance function $k(\cdot, \cdot)$:  
\begin{equation}
...
At a given point $\bx \in \iD_bX$ the realization of the GP is a Gaussian random variable $G(\bx) \sim \iN(m(\bx), k(\bx, \bx))$. 
...
To deal with high dimensional problems, unimportant inputs can be screened using sensitivity analysis (see Section~\ref{sec:screening}), otherwise, model order reduction methods might be used \elias{ref ?}. 
