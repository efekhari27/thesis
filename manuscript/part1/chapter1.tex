...
For example, imprecise probability theory allows more general modeling of the uncertainties \citep{beer_2013_imprecise_proba,schobi_2017_thesis,ajenjo_2023}. 
...
Appropriate data visualization helps to select the fitting methods that are likely to be relevant to the problem. 
...
\begin{itemize}
    ...
    \item Is the support restrictive? If so, specific families of parametric distributions with bounded support can be chosen or truncation can be applied;
...
A \textit{quadrature rule} selects the input samples $\bX_n$ (also called ``nodes'' in classical numerical integration), and an associated set of weights $\bw_n = \{w_1, \dots, w_n\} \in \R^n$. 
...
In the context of a costly numerical model, defining the quadrature rule ahead allows the analyst to massively distribute the evaluations of the numerical model. 
...
To overcome this practical drawback, \citet{fejer_1933} then \citet{clenshaw_1960} proposed a nested rule with mostly equivalent accuracy as Gaussian quadratures.
This method is usually presented to integrate a function w.r.t. the uniform measure on $[-1, 1]$ and starts with a change of variables, $x:=\cos(\theta), \, \dd x := -\sin(\theta) \dd \theta$:
\begin{equation}
...
A generalized discrepancy between distributions called the \textit{maximum mean discrepancy} is introduced in Appendix~\ref{apx:B} and used for efficient sampling in Chapter \ref{chpt:4} of this manuscript.
...
A randomization overlay removes the deterministic property of these designs, allowing us to compute confidence intervals. 
...
Among them, let us mention the ``maximum projection designs'' (MaxPro) from \citet{joseph_gul_2015} which rely on the optimization of a geometrical criterion and deliver interesting performances. 
...
The quantile of order $\alpha$, also called $\alpha$-quantile, denoted by $q_\alpha$(Y), of the output random variable $Y \in \R$ is defined as:
\begin{equation}
...
The design point corresponds to the smallest Euclidean distance between the limit-state surface and the origin of the standard normal space. 
...
The textbook example illustrated in \fig{fig:multi_FORM} shows that the method might miss some significant areas of the failure domain, leading to an estimation error (here, a relative error of 11\% for a failure probability $\pf\approx10^{-3}$). 
...
\end{equation}
This simple writing trick allows one to integrate against the auxiliary distribution. 
...
\begin{equation}
    ...
    \label{eq:pf_IS}
\end{equation}
Similarly to Monte Carlo, this estimator is unbiased (for any intrumental distribution such that $h_\bX(\bx) > 0$), however, its variance is defined as: 
\begin{equation}
...
More recently, alternative MCMC methods including physical system dynamics (e.g., Hamiltonian MCMC) showed promising results \citep{papakonstantinou_2023_HMCMC}. 
Their main drawback is the use of gradients, which can be expensive in high-dimension reliability problems.
...
\begin{itemize}
    ...
    \item \textit{importance measures}: assess the contribution of inputs to the global variability of the output (i.e., in a quantitative way).
\end{itemize}
...
Beyond the qualitative results from the screening methods, importance measures quantify the influence of inputs, allowing us to rank the inputs according to their contribution to the output variability.  
...
\end{equation}
where $A^{\complement}=\{1, \dots, d\} \setminus A$.
...
\begin{equation}
    ...
    \label{eq:mmd_gsa}
\end{equation}
\citet{daveiga_2021_kernel_ANOVA} remarked that the unnormalized first-order Sobol' indices are recovered by taking the linear kernel on the output, such that $k_{Y}(y, y')=y\, y'$. 
...
This section introduces the GSA methods commonly used in UQ, either to reduce the dimension of a problem (screening) or to quantify the influence of inputs (with importance measures), GSA improves the understanding of a UQ study.   
...
The role of a regulatory agency auditing a UQ approach for certification (i.e., a nuclear safety authority), might be to challenge the modelling of uncertainties on the inputs.  
In this case, various tools for \textit{robustness analysis} exist to quantify the impact of misspecifying the random inputs on the quantity of interest studied. 
...
Leave-One-Out validation (\abv{loo}) is an extreme case of $k$-fold cross-validation, for which $k=n$. 
...
\begin{figure}[ht]
    ...
    The predictor is represented in red and several trajectories of the conditioned GP are drawn and represented in purple.}
...
In other words, it should accurately discriminate the points belonging to the safe domain from those belonging to the failure domain. 
...
\begin{equation}
    \iA^{\textrm{U}}(\bx; \by_n) = \frac{\left|\yth - \eta_n(\bx)\right|}{s_n(\bx)}\, , 
    \label{eq:U}
\end{equation}
