...
Associating an uncertainty quantification (\abv{uq}) analysis with these numerical models is a key element in improving the understanding of the phenomena studied. 
...
Additionally, numerical examples of the important methods are implemented during this chapter using \texttt{OpenTURNS}, a Python package for uncertainty quantification.    


\paragraph*{\href{https://openturns.github.io/www/}{\textbf{\texttt{OpenTURNS}}}\footnotemark.} 
...
Finally, no-code users can benefit from \texttt{\abv{openturns}}'s free-download Graphical User Interface software, named \texttt{Persalys}\footnotemark. 
In this chapter, the methodological concepts introduced are linked to minimal \ot implementations examples, available in Appendix~\ref{apx:D}. 

\footnotetext{\ot installation guide and documentation are available at \url{https://openturns.github.io/www/}}
\footnotetext{\href{https://www.persalys.fr/obtenir.php}{\texttt{Persalys}}, a free-download graphical user interface available at \url{https://www.persalys.fr/obtenir.php}}
...
While simulation models are in most cases deterministic, they may also be qualified as intrinsically stochastic (i.e., two runs of the same model taking the same inputs return different outputs).
...
UQ methods suited to other types of outputs exist (see e.g., for time series outputs \citealp{lataniotis_2019}, for functional outputs \citealp{auder_2012,rollon_2021}). 
Let us then define the following numerical model $\iM$, with its respective input $\iD_{\bx}$ and output domains $\iD_{\by}$:
\begin{equation}
\iM : \bigg|
    \begin{array}{rcl}
...
It is an overlay of code allowing its execution in a parametric way, which is often associated with \textit{high-perfomance computer} (\abv{hpc}) deployment.  
...
%============================================================%
%============================================================%
\section{Enumerating and modeling the uncertain inputs} \label{sec:uq}
%============================================================%
%============================================================%
...
Numerous authors proposed to classify them for practical purposes into two groups:
\begin{itemize}
...
\end{equation}
The \textit{cumulative distribution function} (\abv{cdf}) is a common tool to manipulate random variables. 
...
In the continuous case, one can also define a corresponding \textit{probability density function} (\abv{pdf}) $f_{\bX}: \iD_{\bx} \rightarrow \R_+$  with 
...
\end{equation}
The \textit{standard deviation} $\sigma_{X_j} = \sqrt{\var(X_j)}$ and \textit{coefficient of variation} (\abv{cov}) $\delta_{X_j} = \frac{\E[X_j]}{\sqrt{\var(X_j)}}$ are two quantities directly associated to the two first moments.
...
In general, the single effects of multivariate distributions tend to be well modeled. 
However, modeling the dependence structure underlying a joint distribution is often overlooked.  
To illustrate the importance of this step, \fig{fig:joint_dist_samples} represents three \abv{iid} samples from three bivariate distributions sharing the same single effects (e.g., here two exponential distributions) but different dependence structures. 
...
outlining the links between numerical integration (i.e., Lebesgue integration or central tendency estimation) and the numerical design of experiments. 
...
\fig{fig:univariate_quads} illustrates the nested properties of Fejér-Clenshaw-Curtis quadrature by representing the nodes of quadrature rules with increasing size.  
...
Uncertainty quantification problems are rarely unidimensional, but one can directly build a multivariate quadrature rule by defining the tensor product (also called full grids) of univariate rules. 
...
%\elias{Introduce the recurrent Smolyak formula?}
...
%------------------------------------------------------------%
Monte Carlo (\abv{mc}) methods were initially developed in the 1940s to solve problems in neutronics.  
...
Formally, the ``Vanilla'' Monte Carlo (sometimes called ``crude'' Monte Carlo) method uses a set of i.i.d. samples $\bX_n = \left\{\bx^{(1)}, \dots, \bx^{(n)}\right\}$ following the joint distribution of $\P_{\bX}$. 
...
\begin{equation}
    \var\left(\overline{y}_n^{\textrm{MC}}\right) = \frac{1}{n} \var\left(g(\bX)\right). 
...
Among the methods presented so far, classical deterministic quadratures are subject to the curse of dimensionality while Monte Carlo methods deliver contrasted performances. 
Quasi-Monte Carlo (\abv{qmc}) is a deterministic family of numerical integration schemes with respect to the uniform measure on $[0, 1]$. 
...
\begin{theorem}[Koksma-Hlawka]\label{thm:koksma_inequality}
...
The function's variation $V(g)$ in \eq{eq:koksma_inequality} can be formally defined as the Hardy-Klause variation: 
\begin{equation}
...
\end{equation}
Let us point out that this star discrepancy $D^*(\bX_n)$ is equivalent to the Kolmogorov-Smirnov statistic, verifying whether the design follows a uniform distribution \citep{fang_liu_2018}. 
...
A generalized discrepancy between distributions called \textit{maximum mean discrepancy} is introduced in Appendix~\ref{apx:D} and used for efficient sampling in Chapter \ref{chpt:4} of this manuscript.
...
To illustrate the different repartition and properties of the methods, \fig{fig:quasi_monte_carlo_designs} represents the three Monte Carlo and quasi-Monte Carlo designs (with size $n=256$). 
...
This complementary information is essential to deliver an end-to-end uncertainty quantification, which is missed in deterministic QMC methods.  
\textit{randomized quasi-Monte Carlo} is a method introducing some randomness in QMC in order to compute confidence intervals while benefiting from a low variance. 
A specific review of the randomized (also called ``scrambled'') QMC is proposed by \citet{lecuyer_2018}. 
Various authors recommend the use of randomized QMC by default instead of QMC as a good practice. 
...
Ultimately, quasi-Monte Carlo methods generate powerful integration schemes. 
The Koksma-Hlawka inequality associates an upper bound and a convergence rate to most integrals. 
A randomization overlay fades the deterministic property of these designs, allowing us to compute confidence intervals. 
...
\begin{itemize}
    ...
    \item Some might request a sampling method conserving its properties in any subdomains (i.e., subprojections of the inputs’ domain). 
...
\end{equation}
More space-filling criteria are reviewed in \citet{abtini_2018} and in Appendix A from \citet{daveiga_iooss_2021}. 
...
%------------------------------------------------------------%
Latin hypercube sampling is a method initially introduced in the 70s for numerical integration \citep{mckay_beckman_1979}. 
...
Various contributions proposed a variance, and a central limit theorem to LHS \citep{owen_1992_tcl_lhs}. 
...
\end{equation}
Where $C$ is a positive constant, showing that the LHS usually reduces the variance for numerical integration (proof proposed in \citealp{stein_1987_lhs}). 
Because of its stratified structure, LHS can generate poor designs from a space-filling point of view (see e.g., the illustration in \fig{fig:poor_LHS}). 
...
Among them, let us mention the maximum projection designs from \citet{joseph_gul_2015} which rely on the optimization of a geometrical criterion and deliver interesting performances. 
In the same vein, the uniform projection designs from \citet{sun_2019} is also a method to optimize LHS, this time based on a criterion averaging discrepancies between each pair of marginals.
...
The authors find correspondences between discrepancies with respect to uniformity and discrepancies with respect to the target distribution. 
...
The propagation with the three following goals and contexts was introduced: 
\begin{itemize}
...
However, some studies require to shift the focus toward specific areas of the output random variables. 
...
This section aims to present another type of uncertainty propagation. 
...
Only a small amount of extreme input conditions or an unlikely unfavorable combination of inputs leads to the failure of the system. 
...
Considering the problem of exceeding a safety threshold in \eq{eq:failure_proba}, the system's performance is commonly defined as the difference between the model's output and the safety threshold $\yth\in\R$. 
Formally, the \textit{limit-state function} (\abv{lsf}) is a deterministic function $g:\R\rightarrow\R$ quantifying this performance: 
\begin{equation}
    g(\bx) = \yth - \iM(\bx).
\end{equation}
Depending on the sign of its images, this function splits the input space into two disjoint and complementary domains called  
...
\end{equation}
The border between these two domains is a hypersurface called \textit{limit-state surface}, defined by $\iF_{\bx}^0 := \{\bx \in \iD_\bx | g(\bx) = 0\}$. 
...
Additionally, note that the reliability problem can be the composition of multiple reliability problems, often modeled as a system of problems in series and parallel.
...
Note that failure events are qualified as rare when their failure probability has an order of magnitude between $10^{-2} \leq \pf \leq 10^{-9}$ (see e.g., \citealp{lemaire_2009}). 
...
one can apply the change of variable $\bx = T(\bu)$ to express the reliability problem from \eq{eq:pf} in the standard space: 
\begin{equation}
    \pf = \P(\cg(\bU) \leq 0) 
...
Working in the standard space, the methods first look for this MPFP, denoted $P^*$, with coordinates $\bu^*$, 
...
It is classically solved by gradient descent algorithms (e.g., Abdo-Rackwitz algorithm \citealp{abdo_rack_1991}) but can also use gradient-free techniques (e.g., Cobyla algorithm \citealp{powell_1994}). 
This point defines the smallest Euclidian distance between the limit-state surface and the origin of the standard space. 
...
Then, $P^*$ is the point with the biggest contribution to the failure probability (see the illustration in \fig{fig:FORM_SORM_approx}). 
...
The textbook example illustrated in \fig{fig:multi_FORM} shows that the method might miss some important areas of the failure domain, leading to poor estimations. 
...
Provided that the failure probability is bounded, this estimator converges toward it almost surely according to the law of large numbers. 
...
\end{equation} 
The variance of this estimator can be used to build its confidence interval according to the central limit theorem (similarly to the ones from \eq{eq:MC_IC}). 
...
Third, it is qualified as an embarrassingly parallel method since each of the numerical simulations is independent. 
Finally, it offers strong convergence guarantees and complementary information on the estimation confidence. 
These properties often make Monte Carlo the reference method in rare event estimation benchmarks. 

However, the advantages of this estimator are shadowed by its slow convergence. 
...
\fig{fig:IS_reliability} illustrates in the same two-dimensional case, the estimation by Monte Carlo and importance sampling centered on the design point. 
The points in red reached the failure domain and their number seems insufficient for Monte Carlo.  
...
Let us consider the distribution $h_{\blambda}$, belonging to the parametric family $\iH_\blambda$, defined for a set of parameters $\blambda$ in a parameter space $\iD_blambda\subseteq \R^p$ as: 
\begin{equation}
...
\end{equation} 
The early work of \citet{bucher_1988_AIS} only included normal distributions to minimize the IS variance \abv{wrt} the parameter $\blambda$, using \eq{eq:IS_variance} the optimization simplifies as:
\begin{equation}
...
The ``cross-entropy'' (CE) method uses Kullback-Leibler (\abv{kl}) divergence to optimize importance sampling. 
...
\end{equation}
To directly estimate this expected value, the failure probability $\pf$ should not be too rare, which allows to use of an empirical estimator of the expected value:
\begin{equation}
...
The final instrumental density $h_{\blambda^*_{[k_{\#}]}}$ is then considered for importance sampling as defined in \eq{eq:pf_IS}: 
\begin{equation}
...
CE-AIS is widely used in rare event estimation, as it develops an adaptive technique while conserving the explicit IS variance given in \eq{eq:IS_variance}.  
...
When the failure domain is composed of multiple regions, different improvements of the CE-AIS were proposed. 
...
%------------------------------------------------------------%
The use of multivariate kernel density estimation (\abv{kde}) to approximate the importance sampling optimal density $h_{\mathrm{opt}}$ was introduced in the context of structural reliability by \cite{ang_1992_nis}, latter followed by \citet{zhang_1996_NIS}.  
...
\end{equation} 
The last instrumental density $\what{h}_{[k_{\#}]}$ is finally considered as an approximation of the optimal density for importance sampling introduced in \eq{eq:pf_IS}: 
...
Overall, the NAIS offers more flexibility to infer the optimal importance sampling density. 
This property might suit problems presenting a highly nonlinear limit state function. 
Then, relying on importance sampling still provides an expression of the estimator's variance, by adapting \eq{eq:IS_variance} to the recurrent mechanism in NAIS. 
...
In practice, the performances of NAIS seriously decrease for problems in dimensions larger than ten. 
...
%------------------------------------------------------------%
Although the concept of splitting already existed, the name of \textit{subset simulation} (\abv{ss}) was first introduced by \citet{AuBeck2001} in the structural reliability community. 
...
Following conditional subset samples are generated by \textit{Monte Carlo Markov Chain} (\abv{mcmc}) sampling of 
...
The second assumes the estimations of the conditional probabilities to be independent and tends to underestimate the coefficient of variation. 
...
The Metropolis–Hastings algorithm is widely used as MCMC algorithm for subset simulation, however, it quickly becomes inefficient as the dimension increases.  
Different improvements of the MH are made possible by working in the standard space, such as the ``component-wise'' (or ``modified Metropolis–Hastings''). 
...
The subset simulation is a versatile method, presenting consistent performances even for rare probabilities. 
Its flexibility allows it to deal with highly nonlinear LSF, but its drawbacks arise from the use of MCMC sampling. 
...
\begin{figure}
    ...
    \caption{Illustration of a rare event estimation by subset simulation ($n=4\cdot10^4, p_0 = 0.1$).}
...
The global sensitivity of an output can be explained by different elements: the marginal effects of the inputs, their dependence, and their interactions. 
...
Screening methods allow discriminating the non-influential variables, which can be considered afterward as determinist to simplify the problem.
...
The elementary effect in the direction of the variable $i$ from an OAT trajectory $t$ is expressed as a finite difference: 
\begin{equation}
...
The connections between DGSM and variance-based GSA measures (i.e., Sobol' indices introduced hereafter), revealed bounding properties between DGSMs and Sobol' total indices \citep{roustant_2017}.
...
Beyond this information, importance measures quantify the influence of inputs, allowing us to rank the inputs according to their contribution to the output variability.  
...
The output random variable $Y = g(\bX)$ can be decomposed, according to \citet{hoeffding_1948}, as: 
\begin{equation}
...
Different authors proved that this decomposition is unique by exploiting the orthogonality of the terms of the decomposition \citep{efron_1981,sobol_1993}. 
%\elias{add footnote for any input domain and the dependence see Chastaing these}
Therefore, this decomposition can be transposed in terms of functional decomposition of variance (also called functional analysis of variance or \abv{fanova}):
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:sobol}
\end{equation}
By using the functional decomposition of variance in \eq{eq:fanova}, one can show that the Sobol' indices add up to one: 
\begin{equation}
...
Assessing Sobol' indices for every order becomes complex in medium to high dimensions. 
The \textit{total Sobol' index} $S_j^T$ associated with the variable $j$, see \citet{homma_1996}, quantifies the share of output variance which is explained by all the interactions of the variable $X_j$:
\begin{equation}
...
By analyzing jointly the first and total Sobol' indices, one can get an indication about the decomposition between the marginal and interaction effects. 
Note that the total indices are only equal to the first indices when the model does not present interactions (i.e., purely additive model).
...
Many estimators using the pick-freeze generic scheme were developed to estimate Sobol' indices (e.g., Saltelli's, Jansen's, Martinez's etc.), see further details in Chapter 3 of \citet{daveiga_iooss_2021}. 
Alternatively, the surrogate models were exploited to estimate such sensitivity measures. 
...
The following section presents the \textit{Hilbert-Schmidt independence criterion} (\abv{hsic}), which was initially introduced by \citet{gretton_2006} for dependence testing, and later adapted as a dependence measure in GSA by \citet{daveiga_2015}. 
...
\end{equation}
This quantity is a distance in the Reproducing kernel Hilbert space (\abv{rkhs}) by taking a characteristic kernel (e.g., the Gaussian or Matérn kernel). 
...
Alternatively, the second option considers a couple of random variables $(X_j, Y)$, with probability distributions $\P_{X_j}$ and $\P_Y$, and assumes the RKHS $\iH$  
...
As for other steps of the generic UQ methodology, SA is made more complicated for computationally costly simulation models, hence the use of surrogate models.  
...
Among the methods to pertubate uncertainty models, some remain in the probabilistic framework, such as the ``perturbed-law based indices'' (\abv{pli}) \citep{lemaitre_2015_PLI,iooss_2022_pli}, or on extra-probabilistic methods \citep{ajenjo_2022_structural_safety}. 
...
A very large catalog of regression methods exists, here is a list of the most encountered ones in the field of UQ:
...
The following section will provide a short focus on Gaussian process regression. %\elias{Add BST's table including names/shape/parameters/reference?}

Validating the accuracy and precision of a surrogate model is an important step to guarantee its fidelity with regard to the numerical model. 
...
Validating surrogate models in a small-data context appears to be an important challenge. 
Different validation criteria and techniques exist. 
The \textit{coefficient of validation}, denoted by $R^2$, is the first validation metric that can be directly computed on the learning set:
\begin{equation}
...
The \textit{predictivity coefficient} is an alternative defined as a normalized \textit{integrated square error} (\abv{ise}): 
\begin{equation}
...
In this section, a particular focus is dedicated to Gaussian process (GP) regression which will be useful in the following chapters (GP is also called kriging after the geostatician D.G. Krige). 
...
Let us first define a prior structure $G$ on the function approximating $g(\cdot)$, taken as a Gaussian process with a mean function $m(\cdot)$ and covariance function $k(\cdot, \cdot)$:  
\begin{equation}
...
\begin{itemize}
    ...
    \item \textit{covariance model}: $~k(\bx, \bx')$, usually taken stationary, such that $k(\bx, \bx') = \sigma^2 k_{\mathrm{s}}(\bx - \bx', \btheta)$ with $\sigma^2>0$ and $\btheta \in \R^d_+$.
\end{itemize}
...
At a given point $\bx \in \iD_\bX$ the realization of the GP is a Gaussian random variable $G(\bx) \sim \iN(m(\bx), k(\bx, \bx))$. 
Working with Gaussian variables allows us to easily write conditioning formulas between $G(\bx)$ and the observations $\by_n$. 
This Gaussian variable $G(\bx)$ conditioned on the observations $\by_n$ is sometimes called conditional posterior $G_n(\bx) := (G(\bx) | \by_n) \sim \iN(\eta_n(\bx), s_n^2(\bx))$. 
The well-known ``Kriging equations'' (see e.g., \citealp{rasmussen_2006}) offer its explicit expression:
\begin{equation}
    \left\{
    \begin{array}{ll}
...
This regression model provides important complementary information with the \textit{kriging variance} $s_n^2(\bx)$, reaching zero at the learning points. 
...
\begin{figure}[ht]
    ...
    The predictor is represented in and several trajectories of the conditioned Gaussian process are drawn and represented in purple.}
...
The predictive variance adequation (\abv{pva}) has been introduced to confirm that the kriging variance is reliable (see e.g., \citealp{bachoc_2013}). 
...
However, well-known numerical issues appear during the estimation of the hyperparameters, especially as the learning size increases. 
More specifically, the computation and memory allocation for the variance-covariance matrix is a recurrent issue. 
Multiple techniques solve this issue by applying compression schemes on this matrix, e.g., based on sparse approximations (e.g., Hierarchical Matrices \citealp{geoga_2020_hmat_gp}). 
...
A first surrogate $G_n(\bx) \sim \iN(\eta_n(\bx), s_n^2(\bx))$ is fitted using \eq{eq:kriging}. 
...
This learning criterion is relatively inexpensive and allows a progressive enhancement of the Gaussian process to solve the optimization problem with a limited number of calls to the true function. 
...
For example, active learning procedures using SVM classifiers were adapted to this specific goal \citep{bourinet_2018}. 
...
Once again, three iterations of the active kriging (\abv{ak}) algorithm are illustrated, with the corresponding learning criterion $U$ (to minimize). 
...
Unlike optimization problems, the surrogate is used for uncertainty propagation, meaning that the rare event estimation is the result of the approximation of the LSF (i.e., contour finding) and a sampling techniques. 
...
As an agnostic strategy, \citet{moustapha_ss_2022} recommend starting by applying an AK method (using the learning function $U$) paired with a subset simulation (taking an intermediary probability $p_0 =0.2$). 
...
To deal with high dimensional problems, unimportant inputs can be screened using sensitivity analysis (see Section~\ref{sec:screening}), otherwise, model order reduction methods might be used \citep{schilders_2008}. 
When the function is stochastic, different approaches allow fitting the function and its intrinsic variability \citep{binois_2019_replication,baker_2022_stochastic_surrogates_review,zhu_2023_thesis}. 
...
From uncertainty modeling, uncertainty propagation, and sensitivity analysis to surrogate modeling. 
