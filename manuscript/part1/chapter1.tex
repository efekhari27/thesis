...
\elias{Many sensitivity indices derive from dissimilarity measures.}

\begin{equation}
    \mathrm{HSIC} = \MMD(\bX Y, \bX Y |X_i)^2
\end{equation}
...
\begin{equation}
    Y = g(\bX) = g_0 + \sum_{i=1}^{d} g_i(X_i) + \sum_{i<j}^{d} g_ij(X_i, X_j) + \dots + g_{1 \dots d}(\bX),
    \label{eq:hoeffding}
\end{equation}
...
The total Sobol' index $T_i$, see \elias{Saltelli book 2008}, quantifies the share of output variance which is explained by all the interactions of the marginal $X_i$:
\begin{equation}
    S_{T_i} = 1 - \frac{\var\left(\E[Y|\bX^{-i}]\right)}{\var(Y)} = \frac{\E\left[\var(Y|\bX^{-i})\right]}{\var(Y)},
    \label{eq:sobol_total}
\end{equation}
where $\bX^{-i}$ represents all the marginals from $\bX$ but $X_i$.
